---
title: "COSC6323: Group Assignment"
...
```{r fig_2a_code, message=FALSE, error=FALSE, warning=FALSE, echo=FALSE}
# library(tidyverse)
...
# nodes <- read.csv(paste(path,"/Faculty_GoogleScholar_Funding_Data_N4190.csv", sep=''),
#                   header = TRUE)
#     
# links <- read.csv(paste(path,"/GoogleScholar_paper_stats.csv", sep=''),
#                   header = F)
# 
...
img<-readPNG("/Users/apple/UH-CPL/XD_Human_genom/Data_OSF/Fig2_all.png")
#now open a plot window with coordinates
plot.new()
#specify the position of the image through bottom-left and top-right coords
rasterImage(img,0,0,0.7,1)
```


**Comments:**
These figures demonstrate the growth of the collaboration network from before the start of the HGP until 2015.
Green nodes represents the faculties that belongs to the BIO group, while the magenta nodes represents the CS group.  
The black nodes corresponds to the XD group. Faculty nodes are classified as XD after the first year in which 
...
```{r fig_2b_code, message=FALSE,warning=FALSE, echo=FALSE}
library(stringr)
library(ggplot2)
library(tidyr)
library(dplyr)
...
We calculated Direct XD links between faculties (blue line) and association links mediated by polinators (red line). Orange area marks the HGP project period.

**Conclusions:**
We tried to quantify emergence and centrality of cross-disciplinary scholars in the network during and after HGP. We can notice marked growth during and in the wake of HGP project period, which greatly illustrates the common trend of cross-disciplinarity growth in the field of genomics as time passes. 



### Fig 3 Descriptive statistics for the career data set
```{r fig_3_code, message=FALSE,warning=FALSE, echo=FALSE}
...
- Fig3A shows that typical Fi, either in BIO or CS department,started his/her career in the 1990s. The HGP initiative started in early 1990, so this set of Faculties are an ideal group for this study on the effect of the genomic revolution on cross-disciplinary evolution.
- Fig3B demonstrates that we have significantly higher degree of cross-collaboration with in the XD group, compared to BIO, CS.
...
```{r fig_S1_code, message=FALSE,warning=FALSE, echo=FALSE}
## Paper "Cross-disciplinary evolution of the genomics revolution"
## Fig S1a
## Research reproduced by: Vitalii Z.
## State: In progress..
...
data5 <- read.csv(paste(path,"/GoogleScholar_paper_stats.csv", sep=''),
                  header = F)
...
```{r fig_S2_Additional_code, echo=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
...
img<-readPNG("/Users/apple/UH-CPL/XD_Human_genom/Data_OSF/FigS3_all.png")
#now open a plot window with coordinates
plot.new()
#specify the position of the image through bottom-left and top-right coords
rasterImage(img,0,0,1,0.7)
```
...
```{r fig_S4_code, message=FALSE,warning=FALSE, echo=FALSE}
...
varY<-1990 ## Year of network, highest border
N_size<-3 ## Nodes size
G_limit<-45 ## Max subnet (G comp) size
...
varY0_3<-1995 ## Year of network, lowest border
varY_3<-2000 ## Year of network, highest border
N_size_3<-3
G_limit_3<-18

varY0_4<-2000 ## Year of network, lowest border
varY_4<-2005 ## Year of network, highest border
N_size_4<-3
G_limit_4<-12

varY0_5<-2005 ## Year of network, lowest border
varY_5<-2010 ## Year of network, highest border
N_size_5<-3
G_limit_5<-10

varY0_6<-2010 ## Year of network, lowest border
varY_6<-2015 ## Year of network, highest border
N_size_6<-6
G_limit_6<-4
...
while (length(V(g_comp[[1]]))>=25) {            ## Filtering giant components
    g_comp <- giant_component_extract(net)
    net <- delete.vertices(net, V(net)[V(g_comp[[1]])]) #substrack vetices from orig network
    i<-i+1
}  
...
V(net)$color=gsub("BIO","black",V(net)$color) ## CS will be magenta
V(net)$color=gsub("CS","green",V(net)$color) ## BIO will be green
V(net)$color=gsub("XD","magenta",V(net)$color) ## XD will be black
...
plot(net, edge.color="white", vertex.label=NA, 
     edge.width=0.2, layout=layout.kamada.kawai(net), 
     main=varY)

varY0<-varY0_2 ## Year of network, lowest border
varY<-varY_2 ## Year of network, highest border
N_size<-N_size_2
G_limit<-G_limit_2
...
while (length(V(g_comp[[1]]))>=G_limit) {            ## Filtering giant components
    g_comp <- giant_component_extract(net)
    net <- delete.vertices(net, V(net)[V(g_comp[[1]])]) #substrack vetices from orig network
    i<-i+1
}  
...
varY0<-varY0_3 ## Year of network, lowest border
varY<-varY_3 ## Year of network, highest border
N_size<-N_size_3
G_limit<-G_limit_3
...
varY0<-varY0_4 ## Year of network, lowest border
varY<-varY_4 ## Year of network, highest border
N_size<-N_size_4
G_limit<-G_limit_4
...
varY0<-varY0_5 ## Year of network, lowest border
varY<-varY_5 ## Year of network, highest border
N_size<-N_size_5
G_limit<-G_limit_5
...
varY0<-varY0_6 ## Year of network, lowest border
varY<-varY_6 ## Year of network, highest border
N_size<-N_size_6
G_limit<-G_limit_6
...
V(net)$size=N_size ##
V(net)$color=V(net)$XDIndicator ## Assigning the "XDIndicator" attribute as the vertex color
V(net)$color=gsub("BIO","magenta",V(net)$color) ## CS will be magenta
V(net)$color=gsub("CS","green",V(net)$color) ## BIO will be green
...
**Comments:**
Green and magenta nodes represent faculty Fi with BIO and CS affilation respectivly, black nodes represent faculty Fi that by time t collaborated with at least one faculty from the opposite department and thus joined XD group.

**Conclusions:**
FigS4 shows that with time giant component of the F network significanly growing in size, cross-disciplinary nodes also grows in numbers. The amount of small subnetworks reduces significantly during and after HGP period.
